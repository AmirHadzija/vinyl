...
<?php require APPROOT . "/views/inc/navbar.php"; ?>
	...
	<div class="row">
		<div class="col-sm ld_bar hidden">
			<div class="load-bar">
				<div class="bar"></div>
				<div class="bar"></div>
				<div class="bar"></div>
				...
				$(window).scroll(function() {
				    if($(window).scrollTop() == $(document).height() - $(window).height()) {
			           ...
			           console.log(count);
		           		...
		           		$('.ld_bar').show();
						$.ajax({
							url: 'http://localhost/vinyl/boutique/index',
							type: 'POST',
							data: {
								load_more : 1,
								count: count					
							...
							success: function(response){
								...
								if (resp.new_load && resp.new_sales) {
									var load_more = '';
									var modal = '';
									jQuery.each(resp.new_load, function(key, value){

										if (value.on_sale == 'no') {										
											modal += "<div class='modal fade specModal' id='"+value.id+"' tabindex='-1' role='dialog' aria-labelledby='show_detailsTitle' aria-hidden='true'><div class='modal-dialog modal-dialog-centered modal-lg' role='document'><div class='modal-content'><div class='modal-body'><div class='container-fluid'><div class='row'><div class='col-sm-7 popup_item'><img class='mod_img' src='"+value.img_url+"'></div><div class='col-sm-5 popup_item'><div class='row'><div class='col header'><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button><h1>"+value.title+"</h1><h3>"+value.id+"</h3><br><h3>$&nbsp;"+value.price+"&nbsp;<span><sup><a href='http://localhost/vinyl/pages/single?id="+value.id+"'>view full details</a></sup></span></h3></div></div><div class='row'><div class='col footer'><form><div class='form-group'><label for='quantity'><span>Quantity:</span></label><input class='form-control col-sm-3' type='text' name='"+value.id+"' id='modal_quantity_"+value.id+"'; maxlength='2'><span id='quantity_msg_"+value.id+"' class='hidden'></span></div><div class='form-group'><button type='button' class='btn btn-primary popup_btn' id='modal_btn_"+value.id+"'>Submit</button></div><div class='form-group'><p>Available items: "+value.inventory_presented+"</p></div></form></div></div></div></div></div></div></div></div></div>";		

											load_more +="<div class='col-sm-4 ar'><div class='row'><div class='col ar_up'><a href='http://localhost/vinyl/pages/single?id="+value.id+"'><img src="+value.img_url+"></a><a href='#details' data-toggle='modal' data-target='#"+value.id+"'><div class='overlay'><div class='text'><span>QUICK VIEW</span></div></div></a></div></div><div class='row'><a href='http://localhost/vinyl/pages/single?id="+value.id+"'><div class='col ar_down'>	<h1>"+value.title+"</h1><h3>$"+value.price+"</h3></div></a></div></div>";
										} else if(value.on_sale == 'yes') {
											jQuery.each(resp.new_sales, function(keys, sales){
												if (value.id == sales.id_album) {
													modal += "<div class='modal fade specModal' id='"+value.id+"' tabindex='-1' role='dialog' aria-labelledby='show_detailsTitle' aria-hidden='true'><div class='modal-dialog modal-dialog-centered modal-lg' role='document'><div class='modal-content'><div class='modal-body'><div class='container-fluid'><div class='row'><div class='col-sm-7 popup_item'><img class='mod_img' src='"+value.img_url+"'></div><div class='col-sm-5 popup_item'><div class='row'><div class='col header'><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button><h1>"+value.title+"</h1><h3>"+value.id+"</h3><br><h3>$&nbsp;"+value.price+"&nbsp;<span><sup><a href='http://localhost/vinyl/pages/single?id="+value.id+"'>view full details</a></sup></span></h3></div></div><div class='row'><div class='col footer'><form><div class='form-group'><label for='quantity'><span>Quantity:</span></label><input class='form-control col-sm-3' type='text' name='"+value.id+"' id='modal_quantity_"+value.id+"'; maxlength='2'><span id='quantity_msg_"+value.id+"' class='hidden'></span></div><div class='form-group'><button type='button' class='btn btn-primary popup_btn' id='modal_btn_"+value.id+"'>Submit</button></div><div class='form-group'><p>Available items: "+value.inventory_presented+"</p></div></form></div></div></div></div></div></div></div></div></div>";
											

													load_more +="<div class='col-sm-4 ar'><div class='row'><div class='col ar_up'><div class='ribbon'><span>SALE&nbsp;"+sales.precentage+"%</span></div><a href='http://localhost/vinyl/pages/single?id="+value.id+"'><img src="+value.img_url+"></a><a href='#details' data-toggle='modal' data-target='#"+value.id+"'><div class='overlay'><div class='text'><span>QUICK VIEW</span></div></div></a></div></div><div class='row'><a href='http://localhost/vinyl/pages/single?id="+value.id+"'><div class='col ar_down'>	<h1>"+value.title+"</h1><h3>$"+value.price+"</h3></div></a></div></div>";
												}
											});
										}
									...
									$('#album_list').append(load_more);
									$('#modal_app').append(modal);

									var count = $('.ar').length;									
									var gap_div = "<div class='col-sm-4 ar'></div>";
									if (count % 3 == 1) {
										$('#album_list').append(gap_div).append(gap_div);
									} else if (count % 3 == 2) {
										$('#album_list').append(gap_div);
